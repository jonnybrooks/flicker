...
var utils = {
	...
	waitOnImages: function waitOnImages(cb) {
...
var canvas = document.querySelector("#flicker canvas"); // get the canvas element
var ctx = canvas.getContext("2d"); // establish the 2d context
var srcs = Array.prototype.slice.call(document.querySelectorAll("#flicker .sprites img")); // get the array of source sprites
var seekInput = document.querySelector(".seek input"); // get the seek bar

var Flicker = { // Flicker config
	animation: animation,
	playing: true,
	forward: true,
	currentFrame: 0,
...
function advance(nf) {
	if (!Flicker.playing) return Flicker;else if (Flicker.direction === "forward") {
		if (nf >= Flicker.animation.frames.length) {
...
function draw(nf) {
	var sprite = document.querySelector(".sprites img[src=\"flick/" + [Flicker.animation.frames[nf].src] + "\"]"); // get sprite
...
function play(nf) {
	var frame = nf || Flicker.currentFrame;
...
seekInput.addEventListener("input", utils.throttle(seek)); // throttle to 42 -> one frame in ms (approx)

utils.waitOnImages(function (loaded) {
	// wait on the images to load
